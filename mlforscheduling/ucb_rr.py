...
def ucb_rr(jobs, delta=0.001):
    """UCB with Round Robin like exploration.

    Jobs of each types are divided into small pieces of length given by the discretization step.
    A UCB based algorithm is then run over each type, running a piece of the job of the chosen type.
    The UCB algorithm gets reward 1 if the running piece is the last before the job completes and 0
    otherwise. We use the confidence bounds of the exponential distribution.
    ...
    flow_time = 0
    while np.sum(m) < k * n:
        current_type, current_job, duration = select_next_job(
            bound, jobs, m, total_time, n, k
        )
        total_time[current_type] += min(duration, int(current_job / delta) + 1)
        if current_job - delta * duration < 0.0:
        ...
        bound[current_type] = update_confidence_bound(
            bound, m, current_type, total_time, n, k
        )
...
def select_duration(bound, jobs, m, total_time, current_type, n, k):
    ...
    if stop < 0:
        return np.inf
    next_bound = update_confidence_bound(
        bound, m, current_type, total_time + duration, n, k
    )

...
def do_stuff(seed, lambda1, lambda2, n):
    ...
    for algo in [etc_u, etc_rr, rr, ftpp, lsept]:
        flow_times_alg = []
        for sc in lambda1:
            jobs1 = rng.lognormal(mean=sc, sigma=0.75, size=n)
            jobs2 = rng.lognormal(mean=1, sigma=0.75, size=n)
...
np.save("./data/etc_vs_opt-2-jobs-uniform-rebuttal.npy", all_flow_times)
